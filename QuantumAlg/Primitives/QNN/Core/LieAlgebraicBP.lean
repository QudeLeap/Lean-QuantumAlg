...
The earlier `LieAlgebraicVariance` model (in `QuantumAlg.Primitives.QNN.Core.Trainability`)
...
`QuantumAlg.Primitives.QNN.Core.Trainability`.
...
`dim_ℝ(g)`. This identification is formalized by
`finrank_real_realForm_eq_finrank_complex_dla`: `dlaDim gens = finrank ℂ g_ℂ =
finrank ℝ (dlaRealForm gens)`, the real dimension of the DLA's Hermitian real form. -/
