...
Matrix** (`QuantumAlg.qfim`, `Util/QuantumFisher.lean`) to the genuine
**dynamical Lie algebra** dimension, proving Larocca et al.
2021 Theorem 1: the achievable QFIM rank never exceeds `dim(g)`.

The bridge is the real-form structure of the DLA. The circuit's
(Heisenberg-rotated) generators `hₐ` are Hermitian with `i·hₐ ∈ g`; since `g`
is the complexification of its skew-Hermitian real form (the DLA of
skew-Hermitian generators is `*`-closed,
`dynamicalLieAlgebra_conjTranspose_mem`), a Hermitian element of `g` is a
**real** linear combination of any Hermitian basis `B` of `g`
...
/-- **A Hermitian element of the DLA has real coordinates in a Hermitian basis.**
If `Xᴴ = X` and `i·X ∈ g`, then `X = ∑ⱼ rⱼ Bⱼ` with `rⱼ ∈ ℝ`. (The DLA is
`*`-closed, so its Hermitian part is the real span of `B`; concretely,
`ℂ`-coordinates of a Hermitian element in a Hermitian basis are real.) -/
...
/-- **Non-vacuity.** The rank bound applies to the generator family `h = B` (the
DLA Hermitian basis itself): each `Bₐ` is Hermitian and `i·Bₐ ∈ g`. This is a
genuine instance for every simple/full DLA (`su`, `so`, `sp`, `gl`), avoiding
the degenerate-witness defect of the previous overparametrization model. -/
...
/-- The Pauli-X matrix on a two-dimensional computational basis. -/
def pauliX2 : Matrix (Fin 2) (Fin 2) ℂ := fun i j => if i = j then 0 else 1

/-- **The QFIM attains positive rank.** For the single Pauli-`X` generator on
the unit state `|0⟩`, `[F] = [4]`, so `rank[F] = 1 > 0`. This is the
strict-positivity companion to the rank bound: the QFIM is not vacuously zero
(e.g. `qfim 0 = 0`), so the bound `0 < rank[F] ≤ dim g` is non-trivially
exercised. -/
