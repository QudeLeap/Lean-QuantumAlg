...
/-- The Ragone second-moment bundle for the `sp(2ⁿ⁺¹)` qubit family. -/
noncomputable def spSM (n : ℕ) :
    RagoneSecondMoment (spHermBasis (n + 1))
      ((spHermBasis (n + 1)).B (spI0 n)) ((spHermBasis (n + 1)).B (spI0 n)) :=
  RagoneSecondMoment.ofHermitian ((spHermBasis (n + 1)).herm (spI0 n))
...
the loss variance is `1 / dim sp(2ⁿ⁺¹) = 2 / (4ⁿ⁺¹ + 2ⁿ⁺¹)`, vanishing exponentially
[RBS+23, Arxiv_Final.tex:691]. -/
theorem spN_variance_value (n : ℕ) : ((spSM n).variance : ℂ) = 1 / (spDim (n + 1) : ℂ) := by
  rw [(spSM n).variance_eq_gPurity ((spHermBasis (n + 1)).herm (spI0 n))
      ((spHermBasis (n + 1)).herm (spI0 n)) (by rw [spHermBasis_dim]; exact spDim_succ_pos n)]
  simp only [DLAHermBasis.gPurity_basis_elem, one_mul, spHermBasis_dim]

/-- **Concrete exponential barren plateau for `sp(2ⁿ)`.** The qubit-indexed family of circuits whose
dynamical Lie algebra is the symplectic algebra `sp(2ⁿ⁺¹)` (dimension `(4ⁿ⁺¹ + 2ⁿ⁺¹)/2`, realized by
the `θ=+1` Pauli strings) has an exponentially vanishing loss variance — a genuine barren plateau
[MBS+18, maintext.tex:148]. -/
theorem spN_hasBarrenPlateau : HasBarrenPlateau (fun n => (spSM n).variance) := by
  refine ragone_hasBarrenPlateau spSM
