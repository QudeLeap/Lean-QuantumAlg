...
enough" counterpoint to the exponential `gl(2ⁿ)` capstone `fullControllability_hasBarrenPlateau`.

Everything is downstream of the proved reductive variance formula
`RagoneReductive.totalVariance_eq`; the Haar/Schur per-ideal projection stays the named hypothesis
(supplied via the `RagoneSecondMoment.ofHermitian` witness). The genuinely new work is the
`su(2)^{⊕n}` Hermitian orthonormal basis (embedded single-qubit Paulis) and the per-ideal g-purity
computations, both built on the `pauliMat` machinery of `PauliStringDLA`.

The global observable `O = X^{⊗n}` (a separate `O(1/2ⁿ)` plateau attributed to
a non-vendored source) is out of scope: its DLA projection vanishes, so the
formula yields the degenerate `Var = 0`.
-/
...
theorem pauliStr_mul_apply (s t : Fin n → Fin 4) (i k : Fin n → Fin 2) :
  ...
  have hfac : ∀ l, (pauli1 (s l) * pauli1 (t l)) (i l) (k l)
      = ∑ ml : Fin 2, pauli1 (s l) (i l) ml * pauli1 (t l) ml (k l) := fun l =>
    Matrix.mul_apply
...
/-- If two single-qubit Paulis multiply to `ph · σ_c`, then the single-site
Pauli strings multiply to `ph · P_{siteP j c}` (the other legs are
`I·I = I`). -/
...
theorem rtNinv_ne_zero (n : ℕ) : rtNinv n ≠ 0 := by
  intro h; have hm := rtNinv_mul_self n; rw [h, mul_zero] at hm
  exact one_div_ne_zero (pow_ne_zero n (two_ne_zero)) hm.symm
...
theorem casimir_cross_aux {N : ℕ} {numComp : ℕ}
    ...
    (i j : Fin numComp) :
    hsInner (basis i).casimir (basis j).casimir = if i = j then ((basis i).dim : ℂ) else 0 := by
  by_cases h : i = j
  · subst h; rw [if_pos rfl]; exact (basis i).casimir_hsInner_self
...
/-- **A reductive bundle from per-ideal Hermitian bases** (the Haar/Schur projection is supplied as
the rank-one-per-ideal witness, exactly as `RagoneSecondMoment.ofHermitian`). -/
noncomputable def RagoneReductive.ofHermitian {N : ℕ} {ρ O : Matrix (Fin N) (Fin N) ℂ}
  ...
  proj_mem := ⟨fun j => (basis j).gPurity O / ((basis j).dim : ℂ), rfl⟩
  proj_orth := by
    intro j
...
/-- The reductive bundle for the single-qubit-gate family `g = su(2)^{⊕n}` with `O = X₀`. -/
noncomputable def rLocal (hn : 0 < n) : RagoneReductive (localState (n := n)) (localObs hn) :=
  RagoneReductive.ofHermitian n embGens su2EmbHermBasis (fun _ _ hij a b => emb_cross_ortho hij a b)
    localState_herm (localObs_herm hn) su2EmbHermBasis_dim_pos
...
theorem localObs_totalVariance_eq (hn : 0 < n) : (rLocal hn).variance = 1 / 3 := by
  have key : ((rLocal hn).variance : ℂ) = 1 / 3 := by
    rw [(rLocal hn).totalVariance_eq localState_herm (localObs_herm hn) su2EmbHermBasis_dim_pos]
  ...
  have hcast : ((rLocal hn).variance : ℂ) = ((1 / 3 : ℝ) : ℂ) := by rw [key]; push_cast; ring
...
theorem localObs_not_hasBarrenPlateau :
    ¬ HasBarrenPlateau (fun m => (rLocal (n := m + 1) (Nat.succ_pos m)).variance) := by
  rintro ⟨base, hbase, C, hC, hbound⟩
  obtain ⟨m, hm⟩ := pow_unbounded_of_one_lt (3 * C) hbase
  have hb := hbound m
  simp only [] at hb
  rw [localObs_totalVariance_eq, sub_zero, abs_of_pos (by norm_num : (0 : ℝ) < 1 / 3)] at hb
