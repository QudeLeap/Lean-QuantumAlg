...
/-- **The genuine `so(2ⁿ)` as a `DLAHermBasis`.** The `(4ⁿ − 2ⁿ)/2`
odd-`#Y` (normalized) Pauli strings form a Hermitian Hilbert–Schmidt
orthonormal basis whose span is the orthogonal algebra `so(2ⁿ)` (the
skew-symmetric matrices), the dynamical Lie algebra generated by
`{i·P_s : #Y(s) odd}`. -/
noncomputable def soHermBasis (n : ℕ) : DLAHermBasis (soGens n) where
  ...
  ortho := soB_ortho n
  span_eq := by
    have ha : Submodule.span ℂ (Set.range (soB n)) ≤
      ...
      exact soB_skew n i
    have hspan_eq_so : Submodule.span ℂ (Set.range (soB n)) =
        (LieAlgebra.Orthogonal.so (Fin (2 ^ n)) ℂ).toSubmodule := by
      refine le_antisymm ha (fun A hA => ?_)
    ...
    have hg : (dynamicalLieAlgebra (soGens n)).toSubmodule ≤
      ...
      have hsub : dynamicalLieAlgebra (soGens n) ≤ LieAlgebra.Orthogonal.so (Fin (2 ^ n)) ℂ := by
        ...
        rintro _ ⟨s, hs, rfl⟩
        rw [SetLike.mem_coe, LieAlgebra.Orthogonal.mem_so, Matrix.transpose_smul,
          pauliMat_transpose, prod_ySign_eq, Odd.neg_one_pow hs, neg_one_smul,
          smul_neg]
      intro x hx
      exact hsub hx
    exact le_antisymm hf (hg.trans hspan_eq_so.ge)
...
/-- The Ragone second-moment bundle for the `so(2ⁿ⁺¹)` qubit family, with state and observable the
...
noncomputable def soSM (n : ℕ) :
  ...
  RagoneSecondMoment.ofHermitian ((soHermBasis (n + 1)).herm (soI0 n))
...
(`P_g = 1`), the loss variance is `1 / dim so(2ⁿ⁺¹) = 2 / (4ⁿ⁺¹ − 2ⁿ⁺¹)`, vanishing exponentially in
the qubit count — the orthogonal analogue of the `su(2ⁿ)` reduction
[RBS+23, Arxiv_Final.tex:691]. -/
theorem soN_variance_value (n : ℕ) : ((soSM n).variance : ℂ) = 1 / (soDim (n + 1) : ℂ) := by
  rw [(soSM n).variance_eq_gPurity ((soHermBasis (n + 1)).herm (soI0 n))
      ((soHermBasis (n + 1)).herm (soI0 n)) (by rw [soHermBasis_dim]; exact soDim_succ_pos n)]
  simp only [DLAHermBasis.gPurity_basis_elem, one_mul, soHermBasis_dim]

/-- **Concrete exponential barren plateau for `so(2ⁿ)`.** The qubit-indexed family of circuits whose
dynamical Lie algebra is the orthogonal algebra `so(2ⁿ⁺¹)` (dimension `(4ⁿ⁺¹ − 2ⁿ⁺¹)/2`, realized by
the odd-`#Y` Pauli strings) has an exponentially vanishing loss variance — a genuine barren plateau
[MBS+18, maintext.tex:148]. The dimension grows faster than `2ⁿ`, so this is a consequence of the
proved variance formula [RBS+23, Arxiv_Final.tex:691]. -/
theorem soN_hasBarrenPlateau : HasBarrenPlateau (fun n => (soSM n).variance) := by
  refine ragone_hasBarrenPlateau soSM
    (fun n => (soHermBasis (n + 1)).herm (soI0 n))
    (fun n => (soHermBasis (n + 1)).herm (soI0 n))
    (fun n => by rw [soHermBasis_dim]; exact soDim_succ_pos n)
    (C := 1) zero_le_one ?_ (base := 2) one_lt_two ?_
  · intro n
    simp [DLAHermBasis.gPurity_basis_elem]
  · intro n
    rw [soHermBasis_dim]
    exact_mod_cast two_pow_le_soDim_succ n
