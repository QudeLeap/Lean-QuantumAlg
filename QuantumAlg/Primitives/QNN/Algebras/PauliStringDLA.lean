...
The `4ⁿ` Pauli strings `P_s = σ_{s₁} ⊗ ⋯ ⊗ σ_{sₙ}` (`s : Fin n → Fin 4`, `σ₀=I, σ₁=X, σ₂=Y, σ₃=Z`)
are a Hermitian, Hilbert–Schmidt orthonormal basis of `gl(2ⁿ, ℂ)`; the `4ⁿ − 1`
**non-identity** ones
...
Pauli strings are indexed over the register type `Fin n → Fin 2`; `P_s i j = ∏ₖ σ_{sₖ}(iₖ, jₖ)`.
Orthonormality `⟪P_s, P_t⟫ = 2ⁿ δ_{st}` factorises over the qubits. The `span_eq` of the
DLA is proved
by the trace-codimension argument (the traceless subspace is a Lie subalgebra and equals
`span{non-identity Paulis}` by dimension), avoiding the Pauli multiplication rule.
-/
...
/-- The normalization scalar `1/√(2ⁿ)` is nonzero. -/
theorem rtNinv_ne_zero (n : ℕ) : rtNinv n ≠ 0 := fun h => by
  have hm := rtNinv_mul_self n
  rw [h, mul_zero] at hm
...
/-- The Ragone second-moment bundle for the `su(2ⁿ⁺¹)` qubit family, with both Hermitian witness
operators chosen as the first normalized basis element, given the Schur hypothesis `hSchur` (H2:
`(g⊗g)^g = span{C}`). Because `su(2ⁿ⁺¹)` is **traceless** the centered `(g⊗g)^g` genuinely equals
`span{C}`, so `hSchur` is true in principle — a deferred named input (the general Schur lemma), not
a circular posit. -/
noncomputable def suSM (n : ℕ)
    (hSchur : gTensorGInvariant (suHermBasis (n + 1))
      = Submodule.span ℂ {(suHermBasis (n + 1)).casimir}) :
    RagoneSecondMoment (suHermBasis (n + 1))
      ((suHermBasis (n + 1)).B (suI0 n)) ((suHermBasis (n + 1)).B (suI0 n)) :=
  RagoneSecondMoment.consistencyWitness ((suHermBasis (n + 1)).herm (suI0 n))
    ((suHermBasis (n + 1)).herm (suI0 n)) (suHermBasis_dim_pos n) hSchur

/-- **The `su(2ⁿ)` reduction, witnessed.** For the genuine `su(2ⁿ⁺¹)` dynamical Lie algebra, given
the Schur hypothesis, the loss variance equals `P_g(ρ)·P_g(O) / (2ⁿ⁺¹·2ⁿ⁺¹ − 1)` — the
`SimpleSU.main` reduction on a concrete `g ≃ su(d)` with `d = 2ⁿ⁺¹`
[RBS+23, Arxiv_Final.tex:682]. -/
theorem suN_variance_eq (n : ℕ)
    (hSchur : gTensorGInvariant (suHermBasis (n + 1))
      = Submodule.span ℂ {(suHermBasis (n + 1)).casimir}) :
    ((suSM n hSchur).variance : ℂ)
      = (suHermBasis (n + 1)).gPurity ((suHermBasis (n + 1)).B (suI0 n))
          * (suHermBasis (n + 1)).gPurity ((suHermBasis (n + 1)).B (suI0 n))
          / ((2 ^ (n + 1) * 2 ^ (n + 1) - 1 : ℕ) : ℂ) :=
  SimpleSU.main (suSM n hSchur) ((suHermBasis (n + 1)).herm (suI0 n))
...
theorem suN_variance_value (n : ℕ)
    (hSchur : gTensorGInvariant (suHermBasis (n + 1))
      = Submodule.span ℂ {(suHermBasis (n + 1)).casimir}) :
    ((suSM n hSchur).variance : ℂ) = 1 / ((2 ^ (n + 1) * 2 ^ (n + 1) - 1 : ℕ) : ℂ) := by
  rw [suN_variance_eq n hSchur, DLAHermBasis.gPurity_basis_elem, one_mul]

/-- **Bundle-quantified exponential barren plateau for `su(2ⁿ)`.** For any genuine
`RagoneSecondMoment` family on the concrete `su(2ⁿ⁺¹)` Pauli-string basis, bounded `g`-purity
...
consistency witness. -/
theorem suN_hasBarrenPlateau
    {ρ O : (n : ℕ) → Matrix (Fin (2 ^ (n + 1))) (Fin (2 ^ (n + 1))) ℂ}
    (M : (n : ℕ) → RagoneSecondMoment (suHermBasis (n + 1)) (ρ n) (O n))
    (hρ : ∀ n, (ρ n)ᴴ = ρ n) (hO : ∀ n, (O n)ᴴ = O n)
    {C : ℝ} (hC : 0 ≤ C)
    (hbound : ∀ n,
      ‖(suHermBasis (n + 1)).gPurity (ρ n) * (suHermBasis (n + 1)).gPurity (O n)‖ ≤ C) :
    HasBarrenPlateau (fun n => (M n).variance) := by
  exact SimpleSU.main_barren_plateau (M := M) hρ hO (fun n => rfl) hC hbound

/-- Schur-parameterized consistency witness for `su(2ⁿ)`. This is the diagonal witness sequence:
once a caller supplies the Schur identity (H2), the variance decays exponentially, but the second
moment itself is `RagoneSecondMoment.consistencyWitness`, not a finite-group Clifford twirl. -/
theorem suN_hasBarrenPlateau_ofSchur
    (hSchur : ∀ n, gTensorGInvariant (suHermBasis (n + 1))
      = Submodule.span ℂ {(suHermBasis (n + 1)).casimir}) :
    HasBarrenPlateau (fun n => (suSM n (hSchur n)).variance) := by
  refine suN_hasBarrenPlateau (M := fun n => suSM n (hSchur n))
    (fun n => (suHermBasis (n + 1)).herm (suI0 n))
    (fun n => (suHermBasis (n + 1)).herm (suI0 n))
    (C := 1) zero_le_one ?_
  intro n
