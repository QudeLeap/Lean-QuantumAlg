...
count `M` is the supremum of the rank over the loss landscape, and its
**saturated** value is the supremum over `M`:
`R_μ(M) = ⨆_θ rank[F_μ(M,θ)]`, `R_μ = ⨆_M R_μ(M)`. The QNN is
**overparametrized at `M`** iff every training state has saturated:
`∀ μ, R_μ(M) = R_μ` (Larocca et al. 2021, Def. 1).
...
The supremum constructions carry explicit `BddAbove` obligations: the per-`M`
sup is finite via `rank ≤ M`, and the saturated sup is finite via the
Theorem-1 ceiling — without which the `ℕ`-indexed `⨆` is junk-valued.
-/
...
(Larocca Def. 1). `F μ M θ` is the real QFIM at training state `μ`, parameter
count `M`, parameters `θ`. The bundled hypotheses are the Theorem-1 ceiling
(`rank ≤ dim g`, tied to the genuine QFIM) and the parameter-padding block
structure (freezing the new parameter to `0` exhibits `F μ M θ` as the
top-left block of `F μ (M+1) _`). -/
structure OverparamData (gens : Set (Matrix (Fin N) (Fin N) ℂ)) where
  ...
  /-- **Parameter padding:** freezing the `(M+1)`-th parameter to `0` recovers
  the `M`-parameter QFIM as the top-left principal block. This drives
  monotonicity of the achievable rank. -/
  pad_block : ∀ μ M θ,
...
/-- **Monotonicity of the achievable rank** — more parameters cannot decrease
it. Proved (not assumed) from the parameter-padding block structure:
`rank[F μ M θ] = rank` of a principal submatrix of `F μ (M+1) (θ,0)`, and a
submatrix's rank is at most the whole. -/
...
QNN stays overparametrized for every `M' ≥ M`. -/
...
`{M | IsOverparametrized M}` is nonempty: take the (finite) max over the
training set of the per-state saturation points. -/
...
/-- The QNN is overparametrized at its critical count (the degeneracy guard:
`M_c` is meaningful). -/
...
parameter-padding-compatible QFIM family over the fully-controllable `gl(1)`
algebra (`dim g = 1`). It instantiates `OverparamData` with a genuine
`Matrix.rank` and attains positive saturated rank — `R = 1 > 0`, `M_c = 1` —
so it is *not* the degenerate all-zero instance (the VAC-1 defect of the
previous overparametrization model). -/
